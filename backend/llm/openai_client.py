"""
DataPilot MCP — OpenAI API Client
Wraps the OpenAI SDK with the same standard interface as groq_client.
...
def complete(
    ...
    logger.debug("OpenAI request: model=%s, messages=%d", chosen_model, len(messages))
    ...
    logger.debug("OpenAI response: %d chars, %d tool_calls", len(content), len(tool_calls))
